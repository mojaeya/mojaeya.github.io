...
**그 형식인 XML**과 **JSON**은 *HTML*처럼 데이터를 보여주는 목적이 아닌, **데이터를 저장하고 전달할 목적**으로만 만들어졌다.
...
실제 강아지 데이터는 식빵, 웰시코기, 1, 2.14 이며, `<dog>, <name>, <family>` 등이 실제 데이터가 무엇을 의미하는지 나타내는 `메타 데이터`이다.  

...
만약 바닐라 자바스크립트로 서버로부터 데이터를 가져와서 document.querySelector, innerText 등으로 출력해준다고 생각하는 대비 너무 편해졌다. (vue에서는 document 객체를 쓸일이 없다!)
...
<div>
  <div
    class="container"
    v-bind:class="{ 'text-red': hasError, active: isActive }"
  >
    클래스 바인딩
  </div>
